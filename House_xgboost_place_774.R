...
train.xg <- train.dummy.out %>% dplyr:: select(-SalePrice)

train.xg <- data.matrix(train.xg)
train.Sale.xg <- train.dummy.out %>% dplyr:: select(SalePrice)
...
bstSparse <- xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.15)
## xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.1)     train_rmse 10000 - 0.0797
## xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.15)    train_rmse 10000 - 0.078758 
## xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.14)    train_rmse 10000 - 0.078638 
## xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.13)    train_rmse 10000 - 0.078499 
## xgboost(data = train.xg, label = train.Sale.xg, seed=10, booster="gblinear", max_depth=10, nround = 10000, alpha=0.11)    train_rmse 10000 - 0.078238 
...
pred.train <- exp(predict(bstSparse, train.xg))
View(pred.train)

test.xg <- test.dummy %>% dplyr:: select(-SalePrice) %>% data.matrix() 
View(test.xg)

pred <- exp(predict(bstSparse, test.xg))
View(pred)
train2 <- cbind(train.dummy.out$SalePrice,exp(predict(bstSparse, train.xg)))
...
write.table(pred, file="xgboost5_alpha0_15.csv", dec=".", sep=",", row.names=FALSE)
...
## Cross validation
library(DAAG)
CV.model <- cv.lm(train.dummy.out, form.lm = formula(log(SalePrice) ~ .), m=2, dots =
